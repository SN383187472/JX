...
logger = logging.getLogger('scanner.nginx')
...
class NginxScanner(BaseScanner):
    ...
    async def connect(self) -> bool:
        try:
            ...
            if self._connected:
                self.logger.info(f'Nginx connect success: {self.host}:{self.ssh_port}')
            else:
                self.logger.error(f'Nginx connect failed: {self.host}:{self.ssh_port}')
    ...
    async def disconnect(self):
        if self._executor:
            ...
            self.logger.info(f'Nginx disconnected: {self.host}')
